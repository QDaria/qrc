#!/usr/bin/env python3
"""
9Q QRC Simulation on Lorenz-63 Attractor
==========================================
Validate QRC on the canonical Lorenz-63 chaotic system.

Lorenz-63 Properties:
- Lyapunov exponent: λ ≈ 0.906
- Lyapunov time: τ_L ≈ 1.104
- Well-studied benchmark for chaotic prediction
"""
...
print("="*80)
print("9Q QRC - LORENZ-63 ATTRACTOR VALIDATION")
print("="*80)
print("System: Lorenz-63 (σ=10, ρ=28, β=8/3)")
print("Expected Lyapunov: λ = 0.906")
print("="*80)
...
with open('../02_new_turbulence/lorenz63_parameters.json', 'r') as f:
    lorenz_params = json.load(f)

print(f"\n✓ Loaded Lorenz-63 trajectory")
print(f"  Shape: {lorenz_data.shape} (timesteps, features)")
print(f"  Features: x, y, z coordinates")
print(f"\nKnown properties:")
print(f"  Lyapunov exponent: λ = {lorenz_params['lyapunov_exponent']:.3f}")
print(f"  Lyapunov time: τ_L = {lorenz_params['lyapunov_time']:.3f}")

# Calculate Lyapunov from data (verification)
calc_lyap_time, calc_lambda = calculate_lyapunov_time(lorenz_data, dt=lorenz_params['dt'])
...
sampled_data = lorenz_data[indices]
...
# Energy correlation (for 3D system, use total energy)
...
horizon_lyapunov = forecast_horizon_in_lyapunov_times(best_r2, lorenz_params['lyapunov_time'])
...
# Save results
results_path = 'lorenz63_9q_results.json'
with open(results_path, 'w') as f:
    json.dump(results, f, indent=2)

# Save predictions
np.save('lorenz63_9q_predictions.npy', y_pred)
np.save('lorenz63_9q_targets.npy', y_test)

print(f"\n✓ Results saved:")
print(f"  {results_path}")
print(f"  lorenz63_9q_predictions.npy")
print(f"  lorenz63_9q_targets.npy")

# ============================================================================
# SUMMARY
# ============================================================================
print("\n" + "="*80)
print("LORENZ-63 QRC VALIDATION - COMPLETE")
print("="*80)
print(f"\n✓ Test R² = {best_r2:.4f}")
