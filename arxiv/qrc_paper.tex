...
\begin{abstract}
We present a quantum reservoir computing (QRC) study on IBM superconducting hardware, comparing 4-qubit and 156-qubit reservoirs executed on the 156-qubit Heron r2 processor \texttt{ibm\_fez} alongside high-fidelity 9-qubit Rigetti simulation employing the Steinegger-Räth (2025) feature engineering methodology. On time-evolving spectral data, we achieve $R^2=0.764$ (4Q, 50 samples) and $R^2=0.723$ (156Q, 200 samples), a qubit count that, to our knowledge, exceeds previously reported QRC hardware experiments (a preliminary 120-qubit superconducting result \cite{yasuda2023quantum} and 108 neutral atoms \cite{kornjaca2024largescale}), although the larger reservoir did not translate into better prediction. To validate QRC generalizability across chaotic regimes, we demonstrate multi-system validation on canonical chaotic attractors: Lorenz-63 ($R^2=0.796$, $\lambda=0.906$) and Rössler ($R^2=0.969$, $\lambda=0.071$), achieving average $R^2=0.908$ across systems spanning a 13$\times$ range in Lyapunov exponents. While these results validate QRC methodology on complex dynamics, direct comparison reveals sample efficiency challenges: the 156-qubit system operates at 1.28 samples/feature versus 5.0 for 4Q. Meanwhile, simulated 9-qubit Rigetti Novera achieves $R^2=0.959$ on the same spectral evolution data through polynomial feature engineering (3,375 features, 0.19 samples/feature with ridge regularization), demonstrating that sophisticated readout strategies can overcome data quality limitations. We identify optimal qubit counts of 8-16 for current data availability and report full-processor QRC on 156-qubit NISQ hardware, with implications for quantum machine learning resource allocation.
\end{abstract}
...
Despite QRC's theoretical promise, a fundamental question remains unaddressed: \textbf{how does QRC performance scale with quantum hardware size?} Early demonstrations used small systems \cite{negoro2018machine, fujii2017harnessing}; hardware work has since reached 108 neutral atoms \cite{kornjaca2024largescale} and a preliminary 120-qubit superconducting result \cite{yasuda2023quantum}, but there is no systematic study comparing performance across orders-of-magnitude qubit count variations. Industry rhetoric suggests that 100+ qubit systems should dramatically outperform smaller processors, justifying investments in large-scale quantum hardware.
...
\textbf{C1: Hardware Scale.} Full-processor QRC on 156 superconducting qubits (\texttt{ibm\_fez}, Heron r2), a larger qubit count than the preliminary 120-qubit superconducting result of Yasuda et al.~\cite{yasuda2023quantum} and the 108-atom neutral-atom experiment of Kornja\v{c}a et al.~\cite{kornjaca2024largescale}; we make no claim of superior learning performance. We achieve $R^2=0.723 \pm 0.022$ on complex time-evolving data.
...
The Steinegger-Räth framework \cite{steinegger2025quantum} combines three multiplicative feature engineering techniques:
...
\begin{itemize}
    \item \textbf{Processor:} 4 qubits of \texttt{ibm\_fez} (IBM Heron r2); job \texttt{d4kdprd74pkc7386uov0}, 27 Nov 2025
    \item \textbf{Circuit:} 8 layers, each a parameterised single-qubit layer followed by a ring-coupled CX entangling layer
    \item \textbf{Readout:} 4 single-qubit $\langle Z_i \rangle$ and 6 two-qubit $\langle Z_i Z_j \rangle$ expectation values (10 features)
    \item \textbf{Shots:} 4{,}000 per circuit, 50 input timesteps; run-time calibration was not archived
\end{itemize}

\subsubsection{IBM 156-Qubit Configuration}

\begin{itemize}
    \item \textbf{Processor:} IBM Heron r2, backend \texttt{ibm\_fez} (156 physical qubits), accessed through the IBM Quantum Platform open plan
    \item \textbf{Topology:} Heavy-hex lattice (degree-2/3 connectivity)
    \item \textbf{Calibration:} run-time calibration was not archived; vendor-published typical Heron r2 values apply
    \item \textbf{Basis Gates:} CZ, ID, RZ, SX, X
    \item \textbf{SWAP Overhead:} 37\% additional gates for non-adjacent ops
\end{itemize}

Run-time calibration data were not archived. The job identifier, date, sample count and shot count of every hardware run are listed in Table~\ref{tab:provenance} (Appendix~\ref{app:hardware_specs}).
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.95\columnwidth]{figures/figure0_circuit_architecture.png}
...
Test $R^2$ & \textbf{0.723 $\pm$ 0.022} & 156-qubit hardware run (\texttt{ibm\_fez}) \\
...
Table~\ref{tab:156q_results} shows 156Q achieves $R^2=0.723$ on time-evolving spectral data with 200 samples and 156 features (1.28 samples/feature). While this validates QRC at 156-qubit scale, the modest sample efficiency ratio reveals resource allocation challenges: 156Q performs comparably to 4Q ($R^2=0.764$) despite 15.6× more features, suggesting diminishing returns from naive qubit scaling without proportional data increases.
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.92\textwidth]{figures/figure7_steinegger_flowchart.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figures/figure3_sample_efficiency.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figures/figure6_lyapunov_comparison.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figures/figure9_spectral_energy.png}
...
\begin{itemize}
    \item QRC can accurately forecast genuine turbulent energy cascades
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figures/figure5_ablation_study.png}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.92\textwidth]{figures/figure8_learning_curves.png}
...
\begin{enumerate}
    \item \textbf{Method Validation}: QRC successfully operates on real 156-qubit hardware
...
This work presents:
\begin{enumerate}
    \item \textbf{Primary Contribution}: Full-processor (156-qubit) QRC on IBM Heron r2 hardware with 200 samples
...
\begin{itemize}
    ...
    \item \textbf{Hardware:} IBM Quantum Platform, open plan (\texttt{ibm\_fez} backend, Heron r2)
    \item \textbf{Data:} Turbulence generated via pseudo-spectral DNS ($64 \times 64$, Re=200)
    \item \textbf{Code and data:} \url{https://github.com/QDaria/qrc} --- reservoir states, sample indices, training spectra, job result files and run scripts (\texttt{data/}, \texttt{hardware/})
\end{itemize}

\noindent Random seeds are fixed for reproducibility. Cross-validation uses stratified splits with seed=42.

\noindent $R^2$ values are scikit-learn \texttt{r2\_score} computed on the flattened test-set spectra (all wavenumbers pooled, variance about the global mean); under this definition the reported 0.764 (4Q) and 0.723 (156Q) reproduce exactly from the deposited data. The per-wavenumber averaged $R^2$ (scikit-learn's default multi-output setting) is lower: 0.32 for 4Q and 0.14 for 156Q. RMSE is 608 (4Q) and 566 (156Q) in spectral-energy units.
...
\textbf{Finding 1:} Full-processor 156-qubit QRC on IBM Heron r2 hardware (\texttt{ibm\_fez}) achieving $R^2=0.723$ (flattened-spectrum definition, Sec.~Reproducibility) on complex time-evolving data. The qubit count exceeds previously reported QRC hardware experiments \cite{yasuda2023quantum, kornjaca2024largescale}; the prediction quality does not.
...
\begin{enumerate}
    \item \textbf{Queue Times}: Wait times on shared open-access backends are variable and outside the experimenter's control
...
IBM Quantum access was through the IBM Quantum Platform open plan; every IBM run in this paper executed on the Heron r2 backend \texttt{ibm\_fez} (job identifiers in Table~\ref{tab:provenance}). Rigetti quantum simulations performed on the qBraid platform with continuous support from the Rigetti team. Simulations performed using Qiskit 0.45 and Qiskit Aer 0.13.

\section*{Code Availability}

All code, data, and figure generation scripts are available at: \url{https://github.com/QDaria/qrc}
...
\subsection{Hardware Run Provenance}
\label{app:provenance}

All IBM hardware executions used the \texttt{qiskit-ibm-runtime} \texttt{SamplerV2} primitive on the Heron r2 backend \texttt{ibm\_fez} through the IBM Quantum Platform open plan. Table~\ref{tab:provenance} lists the job identifiers as recorded in the result files archived with the code (\texttt{hardware/jobs/}). The 200-sample reservoir-state matrix used for the 156Q results (\texttt{data/reservoir\_states\_hero\_156q\_200.npy}, $200 \times 156$) was assembled on 2025-11-30 from the 50-sample run (\texttt{d4m7kg10i6jc73dgg1n0}) and the 150-sample extension (\texttt{d4m87dl74pkc7388nll0}) submitted by \texttt{hardware/scripts/run\_append\_150.py}. Raw measurement counts for the 156Q jobs (130~MB) are archived by the author and available on request; \texttt{hardware/jobs/raw\_counts.json} holds the counts of the unused Rigetti Ankaa-3 attempt.

\begin{table}[!htbp]
\centering
\caption{IBM hardware runs (all on \texttt{ibm\_fez}, 4{,}000 shots per circuit)}
\label{tab:provenance}
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{Run} & \textbf{Job ID} & \textbf{Date (UTC)} & \textbf{Samples} & \textbf{Qubits} \\
\midrule
4Q validation & \texttt{d4kdprd74pkc7386uov0} & 2025-11-27 & 50 & 4 \\
156Q pilot (discarded) & \texttt{d4kqhgh0i6jc73df4690} & 2025-11-28 & 10 & 156 \\
156Q run & \texttt{d4m7kg10i6jc73dgg1n0} & 2025-11-30 & 50 & 156 \\
156Q extension & \texttt{d4m87dl74pkc7388nll0} & 2025-11-30 & 150 & 156 \\
\bottomrule
\end{tabular}
\end{table}

A 36-qubit run on Rigetti Ankaa-3 via qBraid was attempted (ten job identifiers in \texttt{hardware/jobs/checkpoint.json}) but its data were not used; all Rigetti results in this paper are simulations.
